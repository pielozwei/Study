...
public class OrganizationDirectorService extends BaseService {
	...
	public Page<OrganizationDirector> find(Page<OrganizationDirector> page, OrganizationDirector organizationDirector) {
		...
		if (organizationDirector.getEmployee() != null && StringUtils.isNotEmpty(organizationDirector.getEmployee().getId())){
			dc.add(Restrictions.like("employee.id", "%"+organizationDirector.getEmployee().getId()+"%"));
		}
	...
	public int deleteByZGId(String zgid){
		return organizationDirectorDao.deleteByZGId(zgid);
	}
