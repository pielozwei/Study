		...
		top.$.jBox.open("iframe:${ctx}/tag/treeselect?url=" + encodeURIComponent("${url}") + "&module=${module}&checked=${checked}&extId=${extId}&nodesLevel=${nodesLevel}&selectIds=" + $("#${id}Id").val(), "选择${title}", 300, 420, {
			buttons:{"确定":"ok", ${allowClear ? "\"清除\" : \"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				if (v=="ok"){
					...
					$("#${id}Id").val(ids);
					$("#${id}Name").val(names);
					//2016-09-28 By XuHongTao 针对工艺路线以及质量检测部分添加
					
					//if($("${url}") == "/ip/technique/treeData"){
						var StrUrl = "${url}";
						if(StrUrl.indexOf("ip/technique/treeData") > 0){
							$("#gylxtag").val(ids);
						}
						if(StrUrl.indexOf("gzzx/gzzxwh/treeData") > 0){
							$("#gzzxtag").val(ids);
						}
						if(StrUrl.indexOf("ip/technique/wlbmtreeData") > 0){
							$("#wlbmtag").val(ids);
						}
						if(StrUrl.indexOf("cangku/ckbmd/treeData") > 0){
							$("#cktag").val(ids);
						}
					//}
