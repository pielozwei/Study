...
<head>
	...
	<script type="text/javascript">
		...
		function yzgb(){
			...
			window.location.href="${ctx}/quality/zjWttzd/yzgbzbryComplete?"+value;
		}
	</script>
</head>
<body>
	...
	<form:form id="inputForm" modelAttribute="zjWttzd" action="${ctx}/quality/zjWttzd/saveWcqktbzly" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="jcdwFzr" htmlEscape="false" maxlength="255" class="input-xlarge " disabled="true"/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:textarea path="zgTb" htmlEscape="false" rows="4" maxlength="255" class="input-xxlarge " disabled="true"/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">整改完成填报人：</label>
			<div class="controls">
				<form:input path="zgTbr" htmlEscape="false" maxlength="32" class="input-xlarge " disabled="true"/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="form-actions">
			...
			<c:if test="${identity == 'yzgb' }">
			<input id="btnSubmit" class="btn btn-primary" type="button" onclick="yzgb();" value="验证关闭"/>
			</c:if>
