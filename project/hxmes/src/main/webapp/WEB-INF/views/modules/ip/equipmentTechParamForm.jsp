...
<script type="text/javascript">
	$(document).ready(function() {
		$("#csbm").focus();
		var code = '${equipmentTechParam.csbm}';
		$("#inputForm").validate({
		rules : {
			csbm : {
				remote : "${ctx}/ip/equipmentTechParam/checkCsbm?oldCsbm=" + code
			}
		},
		messages : {
			csbm : {
...
<body>
	<ul class="nav nav-tabs">
		<li class="active">
			<a href="${ctx}/ip/equipmentTechParam/form?id=${equipmentTechParam.id}&equipmentModel.id=${equipmentTechParam.equipmentModel.id}">
				技术参数
				<shiro:hasPermission name="ip:equipmentTechParam:edit">${not empty equipmentTechParam.id?'修改':'添加'}</shiro:hasPermission>
				<shiro:lacksPermission name="ip:equipmentTechParam:edit">查看</shiro:lacksPermission>
			</a>
		</li>
		<li class="pull-right">
	...
	<form:form id="inputForm" modelAttribute="equipmentTechParam" action="${ctx}/ip/equipmentTechParam/save" method="post" class="form-horizontal">
		<form:hidden path="id" />
		<form:hidden path="equipmentModel.id" value="${equipmentTechParam.equipmentModel.id}" />
		<form:hidden path="equipmentModel.sbggxhbm" value="${equipmentTechParam.equipmentModel.sbggxhbm}" />
		<tags:message content="${message}" />

		<div class="control-group">
			<label class="control-label" for="csbm">参数编码:</label>
			<div class="controls">
				<form:input path="csbm" htmlEscape="false" maxlength="30" class="required"  />
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="cszlx">
					<form:options items="${fns:getDictList('d_jscszlx')}" itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="csjldw.id">
					<form:options items="${fns:getJldwDictList()}" itemLabel="jldwmc" itemValue="id" htmlEscape="false" />
				</form:select>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:textarea path="bz" htmlEscape="false" rows="4" maxlength="30" class="input-xxlarge" />
			</div>
		</div>
	</form:form>
</body>
