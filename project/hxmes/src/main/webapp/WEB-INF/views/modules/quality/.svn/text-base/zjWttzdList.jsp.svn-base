...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/quality/zjWttzd/">代办任务</a></li>
		<li><a href="${ctx}/quality/zjWttzd/">所有任务</a></li>
		<shiro:hasPermission name="quality:zjWttzd:edit"><li><a href="${ctx}/quality/zjWttzd/form">问题通知单起草</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="zjWttzd" action="${ctx}/quality/zjWttzd/" method="post" class="breadcrumb form-search">
		...
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
			...
			<label>责任单位：</label>
				<form:input path="dwZr" htmlEscape="false" maxlength="255" class="input-medium"/>
			<label>流程状态：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				<th>序号</th><th>流程实例编号</th><th>责任单位</th><th>监督单位</th><th>检查单位</th><th>问题描述</th><th>被监督单位意见</th><th>整改措施及完成情况</th><th>整改完成填报人</th><th>整改完成验证情况</th><th>整改完成验证人</th><th>流程状态</th>
		...
		<c:forEach items="${page.list}" var="zjWttzd">
			<tr>
				...
				<shiro:hasPermission name="quality:zjWttzd:edit"><td>
    				<a href="${ctx}/quality/zjWttzd/form?id=${zjWttzd.id}">修改</a>
