	...
	<script type="text/javascript">
		...
		function search(){
			...
			if(sign == 'jumpQczbyPage'){
				window.location.href = '${ctx}/quality/zjWttzd/jumpQczbyPage?'+search;
			}else if(sign == 'getCompleteTask'){
				window.location.href = '${ctx}/quality/zjWttzd/getCompleteTask?'+search;
			}else if(sign == 'requestList'){
				window.location.href = '${ctx}/quality/zjWttzd/requestList?'+search;
			}
		}
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		...
		<%-- <li><a href="${ctx}/quality/zjWttzd/processDeploy">流程部署</a></li> --%>
		...
		<c:forEach items="${page.list}" var="zjWttzd">
			<tr>
				...
				<shiro:hasPermission name="quality:zjWttzd:edit"><td>
					<a href="${ctx}/quality/zjWttzd/qczbyForm?id=${zjWttzd.id}&identity=lcqd">流程启动</a>
				...
				<c:if test="${sign == 'getCompleteTask'}">
				<shiro:hasPermission name="quality:zjWttzd:edit"><td>
					<a href="${ctx}/quality/zjWttzd/qczbyForm?id=${zjWttzd.id}&identity=cl">处理</a>
    				<%-- <a href="${ctx}/quality/zjWttzd/qczbyForm?id=${zjWttzd.id}">修改</a>
					<a href="${ctx}/quality/zjWttzd/delete?id=${zjWttzd.id}" onclick="return confirmx('确认要删除该质量问题单吗？', this.href)">删除</a> --%>
				</td></shiro:hasPermission>
				</c:if>
