				...
				<fieldset>
					...
					<div class="control-group">
						<table id="contentTable" class="table table-striped table-bordered table-condensed">
							<thead><tr><th title="数据库字段名">列名</th><th title="默认读取数据库字段备注">说明</th><th title="数据库中设置的字段类型及长度">物理类型</th>
								...
								<th title="选中后该字段被加入到查询条件里">查询</th><th title="该字段为查询字段时的查询匹配方式">匹配方式</th>
								<th title="字段在表单中显示的类型">显示表单类型</th><th title="显示表单类型设置为“下拉框、复选框、点选框”时，需设置字典的类型">字典类型</th>
								<th>排序</th></tr></thead>
							<tbody>
							<c:forEach items="${genTable.columnList}" var="column" varStatus="vs">
								<tr${column.delFlag eq '1'?' class="error" title="已删除的列，保存之后消失！"':''}>
									...
									<td>
										<input type="text" name="columnList[${vs.index}].comments" value="${column.comments}" maxlength="200" class="required" style="width:80px;"/>
									</td>
									<td nowrap>
									...
									<td>		
										<input type="text" name="columnList[${vs.index}].queryType" value="${column.queryType}" maxlength="200" class="input-mini"/>
										<!-- 
										<select name="columnList[${vs.index}].queryType" class="required input-mini">
											<c:forEach items="${fns:getDictList('gen_query_type')}" var="dict">
												<option value="${fns:escapeHtml(dict.value)}">${fns:escapeHtml(dict.label)} </option>
											</c:forEach>
										</select> -->
									</td>
									<td>		
										<input type="text" name="columnList[${vs.index}].showType" value="${column.showType}" maxlength="200" class="input-mini"/>
									</td>
									<td>
										<input type="text" name="columnList[${vs.index}].dictType" value="${column.dictType}" maxlength="200" class="input-mini"/>
									</td>
									<td>
										<input type="text" name="columnList[${vs.index}].sort" value="${column.sort}" maxlength="8" class="required input-min digits" style="width:30px;"/>
									</td>
								</tr>
							</c:forEach>
							</tbody>
						</table>
						(匹配方式:= != &gt; &lt; between like left_like right_like)(显示表单类型:input textarea select checkbox radiobox dateselect userselect officeselect areaselect)
					</div>
				</fieldset>
				<div class="form-actions">
